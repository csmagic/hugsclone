/* --------------------------------------------------------------------------
 * output.c:    Copyright (c) Mark P Jones 1991-1996.   All rights reserved.
...
Cell e; {
	...
	case NEGNUM	:
	case ZERONUM	:
	case POSNUM	: xs = bigOut(e,NIL,d>=FUN_PREC);
...
static Void local putFlds(e,fs)		/* Output exp using labelled fields*/
Cell e;
List fs; {
    put(ALWAYS,e);
    putChr('{');
...
Int  co; {
	...
	case NEGNUM	:
	case ZERONUM	:
	case POSNUM	: {   List xs = bigOut(e,NIL,d>=FUN_PREC);
			      for (; nonNull(xs); xs=tl(xs))
				  putChr(charOf(arg(hd(xs))));
			  }
			  break;
